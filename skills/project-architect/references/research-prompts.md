...
This checklist applies to **every dispatch** — phase-level (Kickoff / Vision / Architecture / Stack / Cost — phases 1–5) and ad-hoc red-flag alike. Every research-scout dispatch MUST cover these bases before topic-specific work begins:
...
1a. **Resolve the newest-stable version of every P0 dependency** (language runtime, primary framework, database, build toolchain) as of today — cite the registry/release page + date, never training-data defaults, no RC/beta/alpha/canary/next pins. Deliver each as a `stack.versions.<package>` value for the orchestrator to record (canonical tokens: `next`, `react`, `node`, `python`, `postgres`, `redis`, `biome`, `typescript` — see `references/decision-keys.md` § `stack.versions.*`); they flow into the generated `package.json` / `Dockerfile` / `pyproject.toml` / `docker-compose.yml` / `biome.json`, and audit check 36 flags any generated artifact whose pin was never recorded.
